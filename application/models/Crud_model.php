...
//$this->db->where('sender !=', $current_user);
    ...
    function employee_resignation($employee_id) {

        $job_title_id = $this->db->get_where('employee', array('employee_id' => $employee_id))->row()->job_title_id;
        $level = $this->db->get_where('job_title', array('job_title_id' => $job_title_id))->row()->level;
        $running_year = $this->db->get_where('settings', array('type' => 'running_year'))->row()->description;

        if ($level == 2) {

            /*
              $specialists_students = $this->db->get_where('students_to_specialists', array('employee_id' => $employee_id, 'year' => $running_year, 'active' => 1))->result_array();
              foreach ($specialists_students as $specialists_students_row) {
              $data_2['active'] = '0';
              $this->db->where('students_to_specialists_id', $specialists_students_row['students_to_specialists_id']);
              $this->db->update('students_to_specialists', $data_2);
              }

              $class_routine_specialist = $this->db->get_where('class_routine_specialist', array('employee_id' => $employee_id, 'year' => $running_year, 'active' => 1))->result_array();
              foreach ($class_routine_specialist as $class_routine_specialist_row) {
              $data_3['active'] = '0';
              $this->db->where('class_routine_specialist_id', $class_routine_specialist_row['class_routine_specialist_id']);
              $this->db->update('class_routine_specialist', $data_3);
              }
             */
            ...
            $this->db->update('schedule', $data_2);
        }
        ...
        if ($level == 1) {
            ...
            foreach ($section as $section_row) {
                echo $section_row['teacher_id'] . '-' . $employee_id;
                if ($section_row['teacher_id'] == $employee_id) {
                    $data_4['teacher_id'] = 0;
                    $this->db->where('section_id', $section_row['section_id']);
                    $this->db->update('section', $data_4);
                }

                if ($section_row['assistant_teacher_id'] == $employee_id) {
                    $data_7['assistant_teacher_id'] = 0;
                    $this->db->where('section_id', $section_row['section_id']);
                    $this->db->update('section', $data_7);
                }
            }
        }
        ...
        //die();
        if ($job_title_id == 2) {
            $data_5['teacher_id'] = NULL;
            $this->db->where('teacher_id', $employee_id);
            $this->db->update('class', $data_5);
        }

        //تحديث تاريخ الاستقالة للموظف في جدول المباشرات والاستقالات
        $data_resignations['resignations_date'] = strtotime(date("Y-m-d H:i:s"));
        $this->db->where('employee_id', $employee_id);
        $this->db->update('effective_resignations_date', $data_resignations);
...
//$returned_array = null_checking($data);
    ...
    function change_online_exam_status_to_attended_for_employee($online_exam_send_id = '', $online_exam_id = "") {
